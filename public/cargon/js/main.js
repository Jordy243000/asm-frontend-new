...
$(".ca-offcanvas-close-toggle, .ca-offcanvas-overlay").on('click', function(){
  ...
  $(".ca-offcanvas-overlay").removeClass("ca-offcanvas-overlay-open");
    ...
    $(window).on('scroll',function(){
      ...
      if(scroll < 1){
        $(".stiky").not("[data-header-pinned]").removeClass("scroll-header");
      }else{
...
$("#topBtn4").on('click', function() {
  ...
  // select
  if ($('select:not(.ca-candidature-select)').length && $.fn.niceSelect) {
    $('select:not(.ca-candidature-select)').niceSelect();
  }
  ...
  function slickReady(selector) {
    var $el = $(selector);
    return $el.length > 0 && $el.children().length > 0 && !$el.hasClass('slick-initialized');
  }

  function initSlick(selector, options) {
    if (slickReady(selector)) {
      $(selector).slick(options);
    }
  }

  // Home 1 Slider 
if (slickReady('.ca-hero1-active')) {
...
// Home 1 testimonial
if (slickReady('.slider-main') && slickReady('.sm-slider-nav')) {
$('.slider-main').slick({
  slidesToShow: 1,
  arrows: false,
...
// Home 2 slider (géré par React sur la page d'accueil ASM)
if (slickReady('.slider-for') && slickReady('.slider-nav') && !$('[data-react-hero-slider]').length) {
$('.slider-for').slick({
  slidesToShow: 1,
  slidesToScroll: 1,
  arrows: false,
...
// Slider 3 Active
if (slickReady('.ca-slider-active-3')) {
...
// testimonial 3 
if (slickReady('.ca-testimonial-3')) {
...
// testimonial 4
if (slickReady('.ca-testimonials-4')) {
...
if (slickReady('.ca-brand-slides-2')) {
